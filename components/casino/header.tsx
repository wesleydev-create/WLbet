    ...
    <>
      <header className="flex flex-col sm:flex-row sm:items-center justify-between px-4 py-2 bg-[#111122] border-b border-border sticky top-0 z-50 gap-2 sm:gap-0">
        ...
        <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto">
          ...
          {/* Botão Depósito → abre modal */}
          <button
            onClick={() => setModalOpen(true)}
            className="bg-gold hover:bg-gold-dark text-primary-foreground font-bold text-sm px-4 py-1.5 rounded transition-colors shrink-0"
          >
            Depósito
          </button>
      ...
      {/* Modal de Depósito */}
      <DepositoModal isOpen={modalOpen} setIsOpen={setModalOpen} />
    </>
  );
}